...
class _MainScreenState extends State<MainScreen>
  ...
  void initState() {
    ...
    _contentFocusNode = FocusNode(debugLabel: 'MainContent');
  ...
  void _requestFocusOnCurrentScreen() {
    ...
    Future.delayed(const Duration(milliseconds: 150), () {
      ...
      if (_selectedIndex == _indexScreenIndex &&
        ...
        _contentFocusNode.requestFocus();
        debugPrint(
          '🎯 Main: Fallback - requested focus on content node '
          '(state: index=${_indexScreenKey.currentState}, tv=${_tvChannelsScreenKey.currentState})',
        );
      }
    });
  }
  ...
  /// Main content uchun key handler.
  ///
  /// Sidebar faqat:
  /// - IndexScreen ichida chekka (left boundary) holatidan
  ...
  void _onMenuItemSelected(int index) {
    // Katalog va Sevimlilar vaqtincha yo‘q
    if (index > 1) {
      if (mounted) {
