...
// Route observer for restoring focus when returning to this screen.
...
class _MainScreenState extends State<MainScreen>
  ...
  KeyEventResult _handleExitMenuKeyEvent(FocusNode node, KeyEvent event) {
    // Barcha event turlarini qabul qilamiz
    if (event is! KeyDownEvent &&
        event is! KeyRepeatEvent &&
        event is! KeyUpEvent) {
      return KeyEventResult.ignored;
    ...
    // Back tugmalari uchun - har qanday event turida ishlaydi
    if (_isBackKey(key)) {
      _ignoreNextWillPop = true; // WillPopScope qayta ochmasin
      _closeExitMenu();
      return KeyEventResult.handled;
    }

    // Left arrow ham exit menuni yopadi
    if (key == LogicalKeyboardKey.arrowLeft) {
      _closeExitMenu();
      return KeyEventResult.handled;
    }

    // Qolgan tugmalar faqat KeyDown va KeyRepeat da ishlaydi
    if (event is! KeyDownEvent && event is! KeyRepeatEvent) {
      return KeyEventResult.ignored;
    }
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            ...
            // Exit confirmation overlay (right sidebar)
