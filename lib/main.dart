...
class _MainScreenState extends State<MainScreen>
  ...
  static const Duration _exitAnimationDuration = Duration(milliseconds: 220);
  static const double _sidebarCollapsedWidth = 72.0;
  static const double _sidebarExpandedWidth = 240.0;

  // State variables
  int _selectedIndex = 0;
  int _sidebarHighlightedIndex = 0;
  bool _isSidebarExpanded = false;

  // Screen keys (IndexScreenContentState / TVChannelsScreenState)
  ...
  // Focus nodes
  late FocusNode _contentFocusNode; // Root content (faqat "shim" sifatida)
  ...
  void initState() {
    ...
    // MUHIM: dastlabki fokusni root Focus'ga emas,
    // hozirgi ekranga (IndexScreen) beramiz
  ...
  void _setupFocusNodes() {
    _contentFocusNode = FocusNode(debugLabel: 'MainContentRoot');
    _sidebarFocusNode = FocusNode(debugLabel: 'Sidebar');
  ...
  void _requestFocusOnCurrentScreen() {
    ...
    if (_selectedIndex == _indexScreenIndex &&
      ...
      _debugLog(
        '⚠️ Main: Fallback - requested focus on root content node '
        '(indexState=${_indexScreenKey.currentState}, '
        'tvState=${_tvChannelsScreenKey.currentState})',
      );
  ...
  /// Root content uchun key handler.
  /// Bu yerda faqat sidebar ochish uchun chap tugma ishlatiladi.
  KeyEventResult _handleContentKeyEvent(FocusNode node, KeyEvent event) {
    ...
    // Qolgan tugmalar child ekranlarga ketadi
    return KeyEventResult.ignored;
  }
    ...
    if (key == LogicalKeyboardKey.arrowDown) {
      setState(() {
        _sidebarHighlightedIndex =
            (_sidebarHighlightedIndex + 1) % _menuItems.length;
      });
      return KeyEventResult.handled;
    }
  ...
  void _onMenuItemSelected(int index) {
    // Katalog va Sevimlilar hozircha yo‘q
    if (index > 1) {
      if (mounted) {
    ...
    // Agar hozirgi sahifani tanlasa – faqat sidebar yopilsin
  ...
  void _openExitMenu() {
    ...
    _exitMenuController.forward().then((_) {
  ...
  void _closeExitMenu() {
    ...
    _exitMenuController.reverse().then((_) {
      if (!mounted) return;
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: Row(
              children: [
                Icon(
                  isCurrentScreen ? item['activeIcon'] : item['icon'],
                  color: Colors.white,
                  size: 24,
                ),
                if (_isSidebarExpanded) ...[
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: GoogleFonts.roboto(
                        ...
                        fontSize: 14,
                        fontWeight:
                            isCurrentScreen ? FontWeight.w600 : FontWeight.w400,
                      ),
                      overflow: TextOverflow.fade,
                      maxLines: 1,
                    ),
      ...
      builder: (context, child) {
        final width =
            ...
            (_expandAnimation.value *
                (_sidebarExpandedWidth - _sidebarCollapsedWidth));
        return Container(
          width: width,
        ...
        body: Stack(
          children: [
            ...
            FocusTraversalGroup(
              ...
              child: Focus(
                // MUHIM: bu yerda endi autofocus=false
                autofocus: false,
